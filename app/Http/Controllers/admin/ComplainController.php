...
class ComplainController extends Controller
{
    ...
    public function solveConflict(Request  $request)
    {
        ...
        if ($complain->percentage < 1) {
            ...
            $order->status = 0; //project is not active
        } else {
            // $order->status = 1;
            $order->status = 2; //project delivered
        ...
        $complain->sal_offer->sal_project_id->delivery_date = \Carbon\Carbon::now();
        ...
        if ($complain->save() &&  $complain->sal_offer->sal_project_id->save()) {
            ...
            if (Auth::check()) {
                ...
                return redirect('/admin/Complains/unsolved')->with(['success' => 'تمت العملية بنجاح']);
            }
        }
