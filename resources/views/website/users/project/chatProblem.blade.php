                    ...
                    <div class="card m-0">
                        ...
                        <div class="row no-gutters">
                            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                                ...
                                {{-- @role('Admin')
                                    <a href="{{ route('chats_solve', $project_id) }}"><button style="color:black ;border:none"
                                            type='submit ' class="note">تم حل المشكله</button></a>
                                @endrole --}}
                                ...
                                <div class="chat-container">
                                    <ul class="chat-box chatContainerScroll" id="chat_body">
                                        ...
                                        @foreach ($chat as $data)
                                            <span style="display: none"
                                                id="receiver">{{ $data->receiver_chat->id }}</span>
                                            <span style="display: none"
                                                id="receiver2">{{ $data->receiver_chat2->id }}</span>
                                            <span style="display: none" id="receiver3">{{ $data->sender_chat->id }}</span>

                                            @if (Auth::user()->id == $data->sender_id)
                                                <li class="chat-left">
                                                    <div class="chat-avatar">
                                                        <img src="{{ Auth::user()->image }}" alt="Retail Admin">
                                                        @if ($data->sender_chat)
                                                            <div class="chat-name">{{ $data->sender_chat->name }}
                                                            </div>
                                                        @endif
                                                    </div>
                                                    <div class="chat-text"> {{ $data->message }}</div>
                                                    <div class="chat-hour">{{ $data->created_at }}</div>
                                                </li>
                                            @elseif(Auth::user()->id == $data->receiver_id1)
                                                <li class="chat-right">
                                                    ...
                                                    <div class="chat-avatar">
                                                        ...
                                                        <img src="{{ $data->sender_chat->image }}" alt="Retail Admin">
                                    ...
                                    @if ($type == 2)
                                        <div class="form-group mt-3 mb-0">
                                            ...
                                            <textarea class="form-control" rows="3" cols='1000' id="chatMassage" placeholder="اكتب هنا الرسالة"
                                                style="width:100%"></textarea>
                                        </div>
                                        <button type="button" class="send_btn"
                                            onclick="send({{ $project_id }})">إرسال</button>
                                    @endif
                                </div>
                            </div>
        ...
        function send(id) {
            ...
            $.ajax('/chatSend', {
                data: {
                    ...
                    'receiver_id2': receiver2,
                    'receiver_id3': receiver3,
                    'message': message,
                    'id': id
                },
        ...
        channel.bind('chatProblem', function(data) {
            ...
            if (current_user == data.sender_id) {
                ...
                var html = '<li class="chat-left">' +
                    '  <div class="chat-avatar">' +
                    '<img src="https://www.bootdey.com/img/Content/avatar/avatar3.png" alt="Retail Admin"><div class="chat-name">' +
                ...
                var html = ' <li class="chat-right">' +
                    ...
                    ' <div class="chat-name">' + data.recever_name1 + '</div><div class="chat-hour">' + output +
                    ' <span class="fa fa-check-circle"></span></div> </div> </li>';
            } else if (current_user == data.receiver_id2) {
                var html = ' <li class="chat-right">' +
                    ' <div class="chat-text">' + data.message + '</div> <div class="chat-avatar">' +
                    ' <img src="https://www.bootdey.com/img/Content/avatar/avatar3.png" alt="Retail Admin">' +
                    ' <div class="chat-name">' + data.recever_name2 + '</div><div class="chat-hour">' + output +
                    ' <span class="fa fa-check-circle"></span></div> </div> </li>';
            }
